...
do{
  ...
  if(priv->idx_size>0 && priv->idx_pos<priv->idx_size){
    ...
    pos = (quad_t)priv->idx_offset+AVI_IDX_OFFSET(idx);
...
static demuxer_t* demux_open_avi(demuxer_t* demuxer){
  ...
  if(priv->idx_size>0){
      ...
      for(i=0;i<priv->idx_size;i++){
        ...
        quad_t pos = priv->idx_offset + AVI_IDX_OFFSET(idx);
                           ...
                           float audio_delay, int flags)
{
  ...
  //FIXME: quad_t - Didn't check AVI case yet (avi files can't be >2G anyway?)
  //================= seek in AVI ==========================
