...
static int mov_check_file(demuxer_t* demuxer){
	...
	int i;
	int skipped=8;
	quad_t len=stream_read_dword(demuxer->stream);
...
//	case MOV_FOURCC('c','m','o','v'):
	  ...
	  priv->moov_start=(quad_t)stream_tell(demuxer->stream);
	  priv->moov_end=(quad_t)priv->moov_start+len-skipped;
...
static int lschunks_intrak(demuxer_t* demuxer, int level, unsigned int id,
                           quad_t pos, quad_t len, mov_track_t* trak);
...
static void lschunks(demuxer_t* demuxer,int level,quad_t endpos,mov_track_t* trak){
    mov_priv_t* priv=demuxer->priv;
...
//	    int temp=stream_read_dword(demuxer->stream);
	...
	case MOV_FOURCC('u','d','t','a'):
	{
		...
		switch (udta_id)
		{
		    ...
		    {
			quad_t text_len = stream_read_word(demuxer->stream);
...
*/
    ...
    case MOV_FOURCC('s','t','s','d'): {
      ...
      mp_msg(MSGT_DEMUX, MSGL_V, "MOV: %*sDescription list! (cnt:%d)\n",
             level, "", count);
      for (i = 0; i < count; i++) {
        quad_t pos = stream_tell(demuxer->stream);
        quad_t len = stream_read_dword(demuxer->stream);
...
if(trak->pos==0 && trak->stream_header_len>0){
    ...
    if (trak) {
      ...
      if (samplenr < 0)
        ...
        quad_t pos = trak->samples[samplenr].pos;
