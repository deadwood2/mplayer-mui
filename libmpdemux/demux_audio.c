...
typedef struct mp3_hdr {
  quad_t frame_pos; // start of first frame in this "chain" of headers
  quad_t next_frame_pos; // here we expect the next header with same parameters
  int mp3_chans;
  int mp3_freq;
  int mpa_spf;
  int mpa_layer;
 ...
 */
static mp3_hdr_t *add_mp3_hdr(mp3_hdr_t **list, quad_t st_pos,
...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  int frmt = 0, n = 0, step;
  quad_t st_pos = 0, next_frame_pos = 0;
