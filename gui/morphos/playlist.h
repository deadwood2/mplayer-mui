...
struct playlist_t
{
	...
	void (*add_track)(playlist_t* playlist, const char *filename, const char *artist, const char *title, int duration, int type);
	...
	void (*sort_playlist)(playlist_t* playlist, int opt, int direction);
...
extern BOOL adddirtoplaylist(playlist_t *playlist, const char* path, BOOL recursive, BOOL refresh_gui);
